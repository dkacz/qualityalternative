You are doing a fresh-from-scratch adversarial audit of Sprint 24 Bedtime Hard Ban R6.
...
2. all prior Pro harvests under `evidence/sprint24_bedtime_hard_ban/pro_review_harvest_r1/` through `pro_review_harvest_r5/`
3. `PRD.md`

Prior Pro results:
- R1/R2/R3/R4/R5 were all `8/10`, `REVISE`, visual `PASS`.
- R5 confirmed the runtime-gate, stale Open Anyway, global-state/settings-emission, and programmatic Pause fixes, then found:
  1. active-Bedtime service interception could still be dropped by foreground duplicate detection after a pre-Bedtime same-package event;
  2. a pre-Bedtime normal intervention did not auto-convert on a pure clock transition without settings emission or user click.

R6 fixes to audit:
- `ForegroundAppDetectionPolicy.shouldLog()` now accepts `bedtimeActive`, tracks whether the previous foreground event was active-Bedtime, and does not treat a same-package event as a duplicate when it crosses from non-Bedtime into active Bedtime.
- `QualityAlternativeAccessibilityService` passes its computed `bedtimeActive` into `ForegroundAppDetectionPolicy.shouldLog(...)`.
- `MainViewModel` now exposes `refreshBedtimeInterventionBoundary()` and the production delay refresh ticker calls it after `refreshActiveDelayWindow()`, so a displayed normal intervention can convert to Bedtime mode without settings emission or a user action.
- Targeted R5 regression logs and a full unit/compile log are included.
...
4. `R1/R2/R3/R4/R5 BLOCKER RECHECK:`
