# Slice 9.2 Review-Fix Validation Transcript

Timestamp: 2026-04-26 00:35:31 CEST

## Commands

```sh
python3 -m json.tool docs/content-sourcing/content_candidate_backlog.schema.json >/dev/null
```

Result: pass.

```sh
python3 - <<'PY'
import csv,json,re
from pathlib import Path
root=Path('docs/content-sourcing')
schema=json.loads((root/'content_candidate_backlog.schema.json').read_text())
fields=[f['name'] for f in schema['fields']]
for name in ['content_candidate_backlog.csv','existing_inventory_audit.csv']:
    with (root/name).open(newline='') as f:
        rows=list(csv.reader(f))
    width=len(rows[0])
    bad=[i+1 for i,row in enumerate(rows) if len(row)!=width]
    if bad:
        raise SystemExit(f'{name}: inconsistent columns at rows {bad[:5]}')
    print(f'{name}: rows={len(rows)} columns={width}')
with (root/'content_candidate_backlog.csv').open(newline='') as f:
    reader=csv.DictReader(f)
    rows=list(reader)
assert reader.fieldnames == fields
assert len(rows)==44
assert sum(r['vertical_slice']=='9.1' for r in rows)==24
assert sum(r['vertical_slice']=='9.2' for r in rows)==20
slice92=[r for r in rows if r['vertical_slice']=='9.2']
assert sum(r['rights_class_candidate']=='RENDERABLE' for r in slice92)==8
assert sum(r['rights_class_candidate']=='LINK_ONLY' for r in slice92)==12
assert all(r['must_not_scrape_cache_or_summarize']=='true' for r in rows if r['rights_class_candidate']=='LINK_ONLY')
assert all((not r['canonical_url_verified_at'] and not r['canonical_url_verified_by']) for r in rows if r['verification_label']!='manually_verified_candidate')
assert all(r['candidate_status']!='approved_for_future_integration' for r in rows)
starter=Path('app/src/main/assets/editorial/starter_packs.json').read_text()
source_urls=set(re.findall(r'"sourceUrl"\s*:\s*"([^"]+)"', starter))
assert not [r['candidate_id'] for r in slice92 if r['canonical_url'] in source_urls]
print('slice9_2_csv_consistency_ok')
PY
```

Result:

```text
content_candidate_backlog.csv: rows=45 columns=80
existing_inventory_audit.csv: rows=46 columns=13
slice9_2_csv_consistency_ok
```

```sh
git diff --check
```

Result: pass.

```sh
JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home \
ANDROID_HOME=/opt/homebrew/share/android-commandlinetools \
ANDROID_SDK_ROOT=/opt/homebrew/share/android-commandlinetools \
./gradlew testDebugUnitTest
```

Result: `BUILD SUCCESSFUL`.

```sh
JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home \
ANDROID_HOME=/opt/homebrew/share/android-commandlinetools \
ANDROID_SDK_ROOT=/opt/homebrew/share/android-commandlinetools \
./gradlew lintDebug
```

Result: `BUILD SUCCESSFUL`.
